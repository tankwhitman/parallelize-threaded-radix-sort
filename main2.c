...
//used a dynamically allocated array so i can simulate having buckets
typedef struct {
  int *array;
  size_t used;
  size_t size;
...
//radix sort algo
void radix_sort (int a[], int n){       //taken with permission from https://www.tutorialspoint.com/c-program-for-radix-sort
...
void* processor()
{
    ...
    writeFD = open("sorted.dat", flags, perms );
    //always going to make 9 buckets
    ...
    int s;
    //always create 9 threads
    thread = calloc(9, sizeof(*thread));
    for(int i = 0; i < 9; i++)
    {
        if((s = pthread_create(&thread[i].threadID, NULL, sorter, &bucket[i]) != 0))
        {
